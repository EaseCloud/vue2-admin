<template>
    <span :class="prefix+'-picker'" :style="styleObject" ref="wapper" v-clickoutside="closeDropdown">
        <span>
            <input :value="label" :placeholder="placeholder" readonly :disabled="disabled"
                   :class="['ant-calendar-range-picker','ant-input',{['ant-input-'+size]:size}]" @click="click"
                   @mousedown="$event.preventDefault()">
            <i v-if="clearable&&label" @click.stop="clear"
               class="anticon anticon-cross-circle ant-calendar-picker-clear"></i>
            <span class="ant-calendar-picker-icon"></span>
        </span>
        <transition name="slide-up">
            <div class="ant-calendar-picker-container"
                 :class="{'ant-calendar-picker-container-placement-bottomLeft':left}" v-show="show" tabindex="-1"
                 @blur="show = false" @mousedown="$event.preventDefault()" @keyup.up="changeMonth(-1,1)" @click.stop
                 @keyup.down="changeMonth(1,1)" @keyup.left="changeYear(-1,1)" @keyup.right="changeYear(1,1)"
                ...
                <div :class="[prefix,{[prefix+'-range']:range},{[prefix+'-time']:showTime || range}]">
                    <div class="ant-calendar-top" v-if="range">
                        <template v-for="(item,index) in ranges">
                            <i v-if="index"></i><a v-text="item.name" :class="item.active?'on':''"
                                                   @click="selectRange(index)"></a>
                        </template>
                    </div>
                        ...
                        <template v-for="no in count">
                            <div :class="range?'ant-calendar-range-part ant-calendar-range-left':''">
                                <div class="ant-calendar-header">
                                    <a class="ant-calendar-prev-year-btn" :title="t('datePicker.lastYear')"
                                       @click="changeYear(-1,no)"></a>
                                    <a class="ant-calendar-prev-month-btn" :title="t('datePicker.lastMonth')"
                                       @click="changeMonth(-1,no)"></a>
                                    <span class="ant-calendar-my-select">
                                        <a class="ant-calendar-year-select" :title="t('datePicker.selectYear')"
                                           @click="showYear(no)">{{$data['now' + no].getFullYear() + t('datePicker.year')}}</a>
                                        <a class="ant-calendar-month-select" :title="t('datePicker.selectMonth')"
                                           @click="showMonth(no)">{{months[$data['now' + no].getMonth()]}}</a>
                                    </span>
                                    <a class="ant-calendar-next-month-btn" :title="t('datePicker.nextYear')"
                                       @click="changeMonth(1,no)"></a>
                                    <a class="ant-calendar-next-year-btn" :title="t('datePicker.nextMonth')"
                                       @click="changeYear(1,no)"></a>
                                </div>
                                    ...
                                    <table class="ant-calendar-table" cellspacing="0" role="grid">
                                        ...
                                        <template v-if="$data['date'+no]">
                                            <tbody class="ant-calendar-tbody">
                                                <tr v-for="i in 6">
                                                    <td v-for="j in 7"
                                                        :title="$data['date'+no][(i-1) * 7 + j - 1].title"
                                                        :class="[prefix+'-cell',$data['date'+no][(i-1) * 7 + j - 1].status]"
                                                        @click="select($data['date'+no][(i-1) * 7 + j - 1], no)">
                                                        <div v-text="$data['date'+no][(i-1) * 7 + j - 1].text"
                                                             :class="prefix+'-date'" aria-selected="false"
                                                             aria-disabled="false">18</div>
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </template>
                                ...
                                <transition name="fade">
                                    <div v-if="showTime" v-show="timeSelected" :class="prefix+'-time-picker'">
                                        <time-picker-panel prefix="ant-calendar-time-picker" v-model="timeVal[no-1]"
                                                           :selected="timeSelected"
                                                           :disabled-h="disabledTime[no-1].disabledHours"
                                                           :disabled-m="disabledTime[no-1].disabledMinutes"
                                                           local-format="HH:mm"
                                                           @close="optionClose"></time-picker-panel>
                                    </div>
                                </transition>
                                <transition name="fade">
                                    <div class="ant-calendar-year-panel" v-if="$data['showYear'+no]">
                                        <span class="ant-calendar-year-panel-prev" @click="changeYearRange(no,-1)"><a
                                          class="anticon anticon-up"></a></span>
                                        <span class="ant-calendar-year-panel-cell"
                                              v-for="(item,index) in $data['years'+no]" :class="item.status"
                                              @click="selectYear(index,no)"
                                              style="width:33.33%; display:inline-block;padding:9px 0">
                                            <a
                                              class="ant-calendar-year-panel-year">{{item.year + t('datePicker.year')}}</a>
                                        </span>
                                        <span class="ant-calendar-year-panel-next" @click="changeYearRange(no,1)"><a
                                          class="anticon anticon-down"></a></span>
                                    </div>
                                </transition>
                                <transition name="fade">
                                        ...
                                        <div class="ant-calendar-month-panel-body">
                                            <table class="ant-calendar-month-panel-table">
                                                <tbody class="ant-calendar-month-panel-tbody">
                                                    <tr v-for="n in 4">
                                                        <td class="ant-calendar-month-panel-cell" v-for="m in 3"
                                                            :class="$data['months'+no][3*(n-1)+m-1].status">
                                                            <a class="ant-calendar-month-panel-month"
                                                               @click="selectMonth(3*(n-1)+m-1,no)">{{months[$data['months' + no][3 * (n - 1) + m - 1].month - 1].substr(0, 3)}}</a>
                                                        </td>
                                                    </tr>
                                                </tbody>
                                            </table>
                    ...
                    <div v-if="range || showTime" :class="[prefix+'-footer',{[prefix+'-range-bottom']:range}]">
                        <component :is="range?'div':'span'" class="ant-calendar-footer-btn">
                            <a v-if="showTime"
                               :class="[prefix+'-time-picker-btn', {[prefix+'-time-picker-btn-disabled']: !timeBtnEnable}]"
                               role="button"
                               @click="selectTime">{{timeSelected ? t('datePicker.selectDate') : t('datePicker.selectTime')}}</a>
                            <a :class="{[prefix+'-ok-btn']: showTime}" role="button"
                               @click="confirm">{{t('datePicker.confirm')}}</a>
                        </component>
                    </div>
                </div>
            </div>
  ...
  export default {
    ...
    methods: {
      ...
      setPosition() {
        ...
        if (this.docking === 'right') {
          this.containerStyle = {
            top: `${p.bottom + 4}px`,
            right: `${this.container.clientWidth - p.right}px`,
          };
        } else { // left or else
          this.containerStyle = {
            top: `${p.bottom + 4}px`,
