  ...
  <div :class="[prefixCls, className ? className : '', hasFooter ? prefixCls + '-with-footer' : '']">
    ...
    <div :class="[prefixCls + '-body', {'ant-transfer-list-body-with-search': showSearch}]">
      <div v-if="showSearch" :class="prefixCls + '-body-search-wrapper'">
        <v-search :prefix-cls="prefixCls + '-search'" :value="filter" @change="handleFilter"
                  :search-placeholder="searchPlaceholder" :handle-clear="handleClear"></v-search>
      </div>
      <ul class="ant-transfer-list-content">
        <template v-for="item in showItems">
          <v-item v-if="item.render" :value="isCheck(item)" @check="select(item)"
                  :item="item" :render="item.render"></v-item>
          <v-item v-else-if="renderItem" :value="isCheck(item)" @check="select(item)"
                  :item="item" :render="renderItem"></v-item>
