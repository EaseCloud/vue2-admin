...
//          }
    ...
    async save () {
      const vm = this;
      const validateResult = await vm.validate().then(() => true, msg => {
        console.log(msg);
        vm.$message.error(msg);
        return false;
      });
      if (!validateResult) return Promise.reject();
      // 保存前置钩子
      if (vm.options.hooks && vm.options.hooks.pre_save
        && !(await vm.options.hooks.pre_save.apply(vm, [vm]))) {
        return Promise.reject();
      }
      const itemToSave = vm.options.hooks && vm.options.hooks.item_filter ?
        await vm.options.hooks.item_filter(vm.item) : vm.item;
      ...
      if (!itemToSave[vm.pk]) {
        ...
        if (redirect !== false) {
          vm.$router.replace({ params: { id: resp.data[vm.pk] } });
        }
      }
      return vm.reload();
    },
